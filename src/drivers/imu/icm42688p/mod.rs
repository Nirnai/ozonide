...
pub struct Icm42688p<SPI, DRDY> {
    ...
    drdy: DRDY,
    config: Config,
    buf: [u8; 15], // buf[0] = cmd byte, buf[1..15] = 14 data bytes
}
...
impl<SPI: SpiDevice, DRDY: Wait> Icm42688p<SPI, DRDY> {
    ...
    pub async fn init(&mut self) {
        ...
        // 200µs required before writing PWR_MGMT0 (datasheet §14.36)
        ...
        // Gyro startup — 30ms typical (datasheet §14.36)
        Timer::after_millis(30).await;

        // INT_CONFIG1: clear INT_ASYNC_RESET (bit 4) — must be cleared after every
        // power-on or soft reset, otherwise INT pin behaviour is undefined.
        self.write_register(INT_CONFIG1, 0b00000000);

        // INT_CONFIG: active high, push-pull, pulsed (~15µs pulse at 1kHz, scope-verified)
    ...
    pub async fn read(&mut self) -> ImuSample {
        let timestamp_us = Instant::now().as_micros();

        // 14-byte burst from IMU_DATA_1(0x1D): [temp×2, accel×6, gyro×6]
        ...
        // Temp: T_°C = raw / 132.48 + 25  (datasheet §4.13)
    ...
    // Reads `len` bytes starting at `reg` into buf[1..=len]. buf[0] holds the command byte.
